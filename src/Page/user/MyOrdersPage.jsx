...
const MyOrdersPage = () => {
  ...
  const fetchMyOrders = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}${ORDER}/order/findByAll/${userKey}`,
        {
      ...
      if (response.data && response.data.statusCode === 200) {
        const orders = Array.isArray(response.data.result)
        ...
        const backendStatusCode = response.data
          ? response.data.statusCode
          : 'N/A';
        const backendStatusMessage =
          response.data && response.data.statusMessage
            ? response.data.statusMessage
            : '예상치 못한 백엔드 오류';
        const httpStatus = response.status;
        if (httpStatus === 404) {
          console.log(
            '주문 목록 가져오기 (404 - 주문 없음):',
            response.data.statusMessage || '주문한 콘텐츠가 없습니다.',
          );
          setOrderList([]);
          setError(null);
        }
      ...
      if (err.response) {
        ...
        const backendErrorMessage =
          err.response.data && err.response.data.statusMessage
            ...
            : '서버 통신 오류';
        const backendStatusCodeInBody =
          err.response.data && err.response.data.statusCode !== undefined
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      ...
      if (response.data && response.data.statusCode === 200) {
        alert(
          response.data.statusMessage || '예매가 정상적으로 취소되었습니다.',
        );
        await fetchMyOrders();
        ...
        setError(
          `주문 취소 실패: ${backendStatusMessage} (상태: ${httpStatus}, 코드: ${backendStatusCode})`,
        );
      } else {
        const httpStatus = response.status;
        console.error(
          '주문 취소 실패 - 예상치 못한 2xx 응답 형식:',
          httpStatus,
          response.data,
        );
        setError(
          `주문 취소 실패: 예상치 못한 서버 응답 형식 (상태: ${httpStatus})`,
        );
      }
    } catch (err) {
  ...
  const handleWriteReviewClick = (contentId) => {
    ...
    alert(
      `"${contentId}" 콘텐츠에 대한 리뷰 작성 페이지로 이동 (아직 기능 없음)`,
    );
  ...
  const handleManageReviewClick = (contentId) => {
    ...
    alert(
      `"${contentId}" 콘텐츠에 대한 리뷰 관리 페이지로 이동 (아직 기능 없음)`,
    );
            ...
            {filteredAndSortedOrders.map((order) => (
              <li key={order.id} className={styles['order-item']}>
                ...
                <div className={styles['order-details']}>
                  ...
                  {/* 현재는 contentId 값만 표시 */}
                  <p>
                    <strong>콘텐츠 ID:</strong> {order.contentId}
                  </p>
                  ...
                  <p>
                    <strong>예매일:</strong>{' '}
                    {order.registDate
                      ? new Date(order.registDate).toLocaleDateString()
                      : '날짜 정보 없음'}
                  </p>
                </div>
                ...
                <p className={styles['order-status']}>
                  ...
                  <span
                    className={
                      order.active === 1 ? styles.active : styles.cancelled
                    }
                  >
