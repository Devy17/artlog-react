...
const UpdatePasswordPage = () => {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    if (newPassword.length < 8) {
      setErrorMessage('변경할 비밀번호는 최소 8자 이상이어야 합니다.');
      return;
    }

    // 새 비밀번호와 현재 비밀번호가 동일한 경우 방지
    if (currentPassword === newPassword) {
      setErrorMessage('변경하려는 비밀번호가 이전 비밀번호와 동일합니다.');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}${USER}/updatePw`, // userId는 Path Variable로 전달
      ...
      if (response.data && response.data.statusCode === 200) {
        // 백엔드 성공 기준 확인 (CommonResDto 사용 시)
        alert(
        ...
        console.error(
          '비밀번호 변경 실패 - 백엔드 비즈니스 오류:',
          response.data,
        );
        setErrorMessage(
          response.data.statusMessage || '비밀랜드 변경 중 오류 발생 (백엔드).',
        );
      }
