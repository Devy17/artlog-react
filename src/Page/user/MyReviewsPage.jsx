...
const MyReviewsPage = () => {
  ...
  const { isLoggedIn, userKey: currentUserKey, authToken, isInit } = authCtx;
const { setModalType } = useContext(ModalContext);
  ...
  const fetchMyReviews = useCallback(async () => {
    ...
    if (!isLoggedIn || !currentUserKey || !authToken) {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        ...
        {
          headers: { Authorization: `Bearer ${authToken}` }, // ✅ Context에서 가져온 authToken 사용
          // 예시: 페이지네이션 파라미터
          // params: {
      ...
      if (response.data && response.data.statusCode === 200 && Array.isArray(response.data.result)) {
        ...
        console.log("리뷰 목록 가져오기 성공:", data); // 성공 로그 추가
  ...
  }, [isLoggedIn, currentUserKey, authToken, currentPage]); // ✅ fetchMyReviews의 의존성에 Context 상태와 페이지 번호 포함
  ...
  useEffect(() => {
    ...
    if (isInit) {
      ...
      if (isLoggedIn) {
        // fetchMyReviews 함수는 useCallback으로 감싸져 있고, 의존성 배열에 필요한 상태를 포함하므로
        // 여기서 fetchMyReviews를 호출하면 상태 변경 시 fetchData 함수가 잘 동작합니다.
        fetchMyReviews();
        // useEffect에서는 초기 로딩 상태만 설정합니다.
        // setLoading(true); // ✅ 이 부분 제거. fetchMyReviews 내부에서 설정
        // setError(null); // ✅ 이 부분 제거. fetchMyReviews 내부에서 설정
      } else {
        ...
        // TODO: 실제 앱에서는 navigate('/login'); 등으로 로그인 페이지로 리다이렉트
      }
    } else {
      // isInit이 false일 때는 Context 초기화 중이므로 로딩 메시지 표시
  ...
  }, [isInit, isLoggedIn, fetchMyReviews]); // ✅ Context 상태와 fetchMyReviews를 의존성에 추가
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    if (!isLoggedIn || !currentUserKey || !authToken) {
      ...
      // TODO: 로그인 페이지 리다이렉트 (선택 사항)
      authCtx.onLogout(); // ✅ Context의 onLogout 사용
      return;
    }

    // ✅ 삭제 시 로딩 상태 업데이트 (선택 사항)
    ...
    try {
      ...
      // ✅ 삭제 로딩 상태 종료 (삭제 시 전체 로딩 스피너를 보여줬을 경우)
      // setLoading(false);
        ...
        <> {/* Fragment를 사용하여 여러 엘리먼트를 그룹화 */}
          ...
          {!loading && !error && (
            reviewData.length > 0 ? (
              ...
              // 로딩 완료, 오류 없음, 데이터 0개인 경우 (이 부분에서 '작성하신 리뷰가 없습니다.' 메시지 표시)
              <p className={styles['info-message']}>작성하신 리뷰가 없습니다.</p>
            )
          )}

          {/* ✅ 페이지네이션 또는 더보기 버튼 (로딩 완료, 오류 없음, 데이터 있을 때만 표시) */}
