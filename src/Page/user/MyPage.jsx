...
const MyPage = () => {
      ...
      if (
        ...
        const backendErrorMessage =
          error.response.data && error.response.data.statusMessage
            ...
            : '서버 오류';
        alert(
          `힌트 키 가져오기 실패: ${backendErrorMessage} (상태: ${error.response.status})`,
        );
      }
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.data && response.data.statusCode === 200) {
        ...
        alert(
          `회원 수정을 실패했습니다.: ${response.data.statusMessage || 'An unknown error occurred.'}`,
        );
      }
    } catch (error) {
      ...
      if (error.response) {
        alert('정보를 입력하세요.');
        console.error(
          `정보 저장을 실패 했습니다. Status: ${error.response.status}. ${error.response.data && error.response.data.statusMessage ? error.response.data.statusMessage : ''}`,
        );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error.response) {
        alert(
          `Failed to delete account. Status: ${error.response.status}. ${error.response.data && error.response.data.statusMessage ? error.response.data.statusMessage : ''}`,
        );
        if (error.response.status === 401 || error.response.status === 403) {
          authCtx.onLogout();
          navigate('/login');
