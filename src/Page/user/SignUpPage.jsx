  ...
  useEffect(() => {
    const fetchHintKeys = async () => {
      try {
        ...
        if (error.response) {
          console.error(
            'Hint keys fetch failed - HTTP Response:',
          ...
          const backendErrorMessage =
            error.response.data && error.response.data.statusMessage
              ...
              : '서버 오류';
          alert(
            `힌트 키 목록 가져오기 실패: ${backendErrorMessage} (상태: ${error.response.status})`,
          );
        } else {
    ...
    try {
      ...
      if (response.data && response.data.statusCode === 200) {
        ...
        if (response.data.result === false) {
          // Assuming false = available
          setIsUserIdAvailable(true);
          alert(`'${userId}'는 사용 가능한 아이디입니다.`);
        } else {
          // Assuming true = unavailable/duplicate
          setIsUserIdAvailable(false);
      ...
      if (error.response) {
        console.error(
          'Duplicate check failed - HTTP Response:',
          error.response.status,
          error.response.data,
        );
        const backendErrorMessage =
          error.response.data && error.response.data.statusMessage
            ...
            : '서버 오류';
        alert(
          `중복 확인 중 오류가 발생했습니다: ${backendErrorMessage} (상태: ${error.response.status})`,
        );
      } else {
      ...
      if (error.response) {
        ...
        const backendErrorMessage =
          error.response.data && error.response.data.statusMessage
            ...
            : '서버 오류';
        alert(
          `회원가입 실패: ${backendErrorMessage} (상태: ${error.response.status})`,
        );
      } else {
        alert(
          '회원가입 요청 중 네트워크 오류가 발생했습니다. 백엔드 서버 상태를 확인해주세요.',
        );
      }
    }
  };
